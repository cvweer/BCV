...
**Your Answer:** Nominal variables: keywords, original_language, title,
genre, first_actor, first_actor_gender, director_first_name,
director_gender

Ordinal variables: NA

Interval variables: index, release_date, release_year, release_month,
release_day

Ratio variables: budget, popularity, revenue, runtime, vote_average,
vote_count
...
mean_profit = 52885669 median_profit = 0 max_profit = 1299557910
min_profit = -1.5e+08
...
**Your Answer:** Bruce Willis is the actor who plays in the most movies
in the data set. We pulled the top 2 rows to confirm that there is not
any other actor with the same amount of movies.
...
**Your Answer:** step 1: cleary state H0 and HA step 2: computation of
the sample variance and length step 3: identify the test statistics step
4: create critical values step 5: conclude H0, the variance of the
runtime is 500 HA, th variance of the runtime in not 500 test statistics
are: H0 =500 and chi_square = ((n-1)*sample_var)/H0 Conclusion: reject
H0, since the variance is significantly different from 500* step b\*
...
**Your Answer:** For the chi square test to be valid, the runtime must
...
**Your Answer:** Bob and Chantal argued about which movies made the
...
lm_profit = (lm(movies4$profits ~ movies4$budget))
summary(lm_profit)
```
...
```{r}
coef(lm_profit)["movies4$budget"]
```

**Your Answer:**

When predicting $\beta_1$ we can conclude one extra dollar spend on
budget is an extra 2.162 euro in profit.

*step c*

```{r}
summary_lm <- summary(lm_profit)

t_value <- summary_lm$coefficients["movies4$budget", "t value"]
p_value <- summary_lm$coefficients["movies4$budget", "Pr(>|t|)"]
...
We choose $\alpha = 0.05$ (5% significance level).

Step 4 – P-value
The one-sided p-value is $3.28 \times 10^{-95}$.

Step 5 – Conclusion
Since the one-sided p-value is far smaller than $\alpha = 0.05$, we reject $H_0$.
This means there is overwhelming evidence that $\beta_1$ is positive (not negative).
So, we conclude that a higher budget is significantly associated with higher profits.
...
```{r}
library(tidyverse)
movies4$mean_log_budget =ifelse(movies4$budget == 0,0, log(movies4$budget))
movies4_g_filter = movies4[!is.na(movies4$first_actor_gender),]



ggplot(movies4_g_filter, aes(x=first_actor_gender, y=mean_log_budget))+
  stat_summary(fun=mean, geom = 'bar')+
  stat_summary(fun.movies4 = mean_cl_normal, geom = 'errorbar', width =.5)
```

**Your Answer:**

from this bargraph i can conclude that movies with a male as first actor on avarage have a higher budget.
*step b*
